...
**Một dòng** (clone → `~/ai-counter`, sandbox → `~/ai-counter-sandbox`, không cần `.env`):
...
Đường dẫn cố định: repo `~/ai-counter`, sandbox `~/ai-counter-sandbox`, container `ai-counter`.

Sau khi cài, đăng nhập một lần:

```bash
HOME=~/ai-counter-sandbox ~/ai-counter/bin/z8l auth login
...
**Thêm project:** clone repo vào `~/ai-counter-sandbox/projects/<tên>` rồi khai báo trong `config.yaml`:

```bash
git clone git@github.com:you/my-app.git ~/ai-counter-sandbox/projects/my-app
```
