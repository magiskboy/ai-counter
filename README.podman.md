...
**Chạy container** (đồng bộ giờ với host — container dùng chung kernel clock, cần khớp **timezone**):

```bash
# Cách 1: script tự lấy TZ từ host + mount /etc/localtime
SANDBOX="$SANDBOX" ./scripts/podman-run.sh
...
podman exec -u counter ai-counter cursor-agent login
```

Đặt `schedule.timezone` trong `~/ai-counter-sandbox/ai-counter/config.yaml` (ví dụ `Asia/Ho_Chi_Minh`) — cron và log dùng múi giờ đó. `-e TZ` khi `podman run` ghi đè config.

```bash
podman exec -u counter ai-counter date   # so sánh với date trên host
```
...
**Cron tự động:** Thứ 2–6, **06:30** theo `schedule.timezone` trong config (mặc định trong `config.example.yaml`: `Asia/Ho_Chi_Minh`).
...
| 9   | **Giờ container lệch host:** dùng `./scripts/podman-run.sh` hoặc `-e TZ=...` + `-v /etc/localtime:/etc/localtime:ro`; kiểm tra `schedule.timezone` trong config.                                          |
